...
<template>
    <div 
        @click="handleClick"
        class="p-4 rounded-xl cursor-pointer transition-all border group relative overflow-hidden mb-3"
        :class="isActive 
    ...
    >
        <div class="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
            <button @click.stop="$emit('edit')" class="p-1 text-slate-400 hover:text-blue-400 transition-colors rounded hover:bg-slate-700/50">
                <div class="i-mdi-pencil text-sm" />
            </button>
        </div>

        <div class="flex justify-between items-center mb-1">
            <h3 class="font-bold text-sm truncate pr-2" :class="isActive ? 'text-blue-400' : 'text-slate-200'">{{ project.name }}</h3>
            <div class="flex-shrink-0">
                 <div v-if="isRunning" class="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)] animate-pulse"></div>
            </div>
        </div>
